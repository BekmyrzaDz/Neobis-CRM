...
const CallMade = () => {
  const CallMadeData = useAppSelector(
    (state) => state.analytics.requestStatus[1]
  )

  const updatedData = updateReqStatusDepartmentNames(CallMadeData)
  ...
  return (
    <div className={styles.callMade}>
      <h2 className={styles.title}>
        {updatedData && updatedData?.name && updatedData.name}
      </h2>
